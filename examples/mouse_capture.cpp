...
void drawRect(int x, int y, int width, int height, float red, float green, float blue)
{
    if (width <= 0 || height <= 0) {
        return;
    }
...
void renderStatusBar(uint32_t framebufferWidth, uint32_t framebufferHeight, const std::string& text)
{
    glViewport(
        0,
        0,
...
int main()
{
    ...
    while (!window.shouldClose()) {
        ...
        renderStatusBar(fbWidth, fbHeight, status);
        window.swapBuffers();
